...
class _casaddState extends State<casadd> {
  ...
  Widget getlocation() {
    return FlatButton(
      child: Text('พิกัดตำแหน่ง = ${locationsget}'),
    );
  }
        ...
        Container(
          ...
          child: RaisedButton.icon(
            ...
            onPressed: () {
              print('คลิ๊กอัปโหลด');
              if (file == null) {
                showAlert('ล้มเหลว', 'กรุณาถ่ายภาพหรืออัปโหลด');
              } else if (name == null ||
                  name.isEmpty ||
                  detail == null ||
                  detail.isEmpty ||
                  selectedType == null|| _longitudeController == null || _latitudeController == null) {
  ...
  Future<void> uploadPicturetofiresStrage() async {
    ...
    print('url= $urlimage');
    InsertvaluetofiresStrage(double.parse(_latitudeController.text),double.parse(_longitudeController.text));
    MaterialPageRoute route = MaterialPageRoute(builder: (value) => casenews());
  ...
  Future<void> InsertvaluetofiresStrage(double lat, double lng) async {
    ...
    _firestore
        ...
        .add({'name': name,'detail': detail,'urlimage': urlimage,'level': selectedType, 'position': geoFirePoint.data}).then((_) {
